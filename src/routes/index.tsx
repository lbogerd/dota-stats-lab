...
import { ArrowRight, ChevronRight, Clock3, Database, FileCode2, Layers3, Plus, TerminalSquare } from "lucide-react";
import { formatRelative, jobsQuery, matchesQuery } from "../web/data";
import { PageHeading, StatusBadge } from "../web/ui";

export const Route = createFileRoute("/")({
  loader: async ({ context }) => Promise.all([context.queryClient.ensureQueryData(jobsQuery()), context.queryClient.ensureQueryData(matchesQuery())]),
  component: Dashboard,
});
...
function Dashboard() {
  ...
  const totalRecords = matches.reduce((sum, match) => sum + match.records, 0);
  return (
    <>
      ...
      <section className="grid grid-cols-2 gap-3 xl:grid-cols-4">
        {[
          { label: "Stored matches", value: matches.length.toString().padStart(2, "0"), note: "+2 this week", icon: Database },
          { label: "Total records", value: `${(totalRecords / 1_000_000).toFixed(1)}M`, note: "Parser-native", icon: Layers3 },
          { label: "Saved queries", value: "03", note: "Durable files", icon: FileCode2 },
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[1.35fr_0.85fr]">
        <div className="card overflow-hidden">
          <div className="flex items-center justify-between border-b border-[#e4e5de] px-5 py-4 sm:px-6"><div><p className="eyebrow">Queue</p><h2 className="mt-1 text-lg font-semibold tracking-[-0.025em]">Active ingestion</h2></div><Link to="/ingest" className="text-xs font-semibold text-[#315f4a]">View all</Link></div>
          ...
          </div> : <div className="p-8 text-center text-sm text-[#778079]">No ingestion jobs are running.</div>}
        </div>
