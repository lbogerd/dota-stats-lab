    ...
    <>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.08fr_0.92fr]">
        ...
        <section className="card overflow-hidden">
          ...
          {active ? <div className="p-5 sm:p-6"><div className="flex items-start justify-between gap-3"><div><p className="font-mono text-lg font-semibold">#{active.matchId}</p><p className="mt-1 text-xs text-[#7d8680]">One job runs at a time</p></div><StatusBadge status={active.status} /></div><div className="relative mt-7 space-y-0">
            {[{ value: "fetching", title: "Replay downloaded", body: "Stored in the durable replay cache" }, { value: "parsing", title: "Parsing replay data", body: "Exporting entity events and checkpoints" }, { value: "loading", title: "Loading extraction", body: "Validating and committing to DuckDB" }].map((step, i) => { const order = ["queued", "fetching", "parsing", "loading", "succeeded"]; const current = order.indexOf(active.status); const done = current > order.indexOf(step.value); const currentStep = current === order.indexOf(step.value); return <div key={step.value} className="relative flex gap-3 pb-6 last:pb-0"><div className={`relative z-10 grid size-7 shrink-0 place-items-center rounded-full border-2 ${done ? "border-[#315f4a] bg-[#315f4a] text-white" : currentStep ? "border-[#315f4a] bg-[#e5ecdf] text-[#315f4a]" : "border-[#d8ddd6] bg-[#fbfaf5] text-transparent"}`}>{done ? <Check size={13} strokeWidth={3} /> : currentStep ? <span className="size-2 rounded-full bg-[#315f4a] pulse-soft" /> : null}</div>{i < 2 && <span className={`absolute left-[13px] top-7 h-[calc(100%-28px)] w-px ${done ? "bg-[#315f4a]" : "bg-[#d8ddd6]"}`} />}<div className="pt-0.5"><p className={`text-sm font-semibold ${!done && !currentStep ? "text-[#9aa19c]" : ""}`}>{step.title}</p><p className="mt-0.5 text-xs text-[#8a928d]">{step.body}</p></div></div>; })}
          </div></div> : <div className="flex min-h-[300px] flex-col items-center justify-center p-8 text-center"><div className="grid size-12 place-items-center rounded-2xl bg-[#e6ecdf] text-[#315f4a]"><Server size={21} /></div><h3 className="mt-4 text-sm font-semibold">The queue is clear</h3><p className="mt-1 max-w-xs text-xs leading-5 text-[#7a837e]">Submit a match and its progress will appear here.</p></div>}
