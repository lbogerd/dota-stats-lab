...
import { formatBytes, formatDuration, formatRelative, matchesQuery } from "../web/data";
...
function MatchesPage() {
  ...
  return <>
    ...
    <div className="card overflow-hidden">
      ...
      <div className="hidden grid-cols-[1.25fr_1.2fr_0.8fr_0.7fr_28px] gap-4 border-b border-[#e2e4dc] bg-[#f4f4ee] px-5 py-2.5 text-[0.62rem] font-bold uppercase tracking-[0.1em] text-[#818a84] md:grid"><span>Match / extraction</span><span>Acquired</span><span>Duration</span><span>Size</span><span /></div>
      <div className="divide-y divide-[#e5e7df]">
        {visible.map((match) => <Link key={match.matchId} to="/matches/$matchId" params={{ matchId: match.matchId }} className="grid gap-3 px-4 py-4 transition hover:bg-white sm:px-5 md:grid-cols-[1.25fr_1.2fr_0.8fr_0.7fr_28px] md:items-center md:gap-4">
          ...
          <div className="flex items-center justify-between md:block"><span className="text-[0.65rem] font-bold uppercase tracking-wider text-[#969d98] md:hidden">Acquired</span><div><p className="text-xs font-medium">{formatRelative(match.acquiredAt)}</p><div className="mt-1"><StatusBadge status="ready" /></div></div></div>
          <div className="flex justify-between text-xs md:block"><span className="font-bold uppercase tracking-wider text-[#969d98] md:hidden">Duration</span><span>{formatDuration(match.durationSeconds)}</span></div>
