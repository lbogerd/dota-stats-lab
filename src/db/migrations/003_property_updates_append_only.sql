-- DuckDB 1.5 can report false duplicate-key violations while bulk-building a
-- composite primary-key ART index for multi-million-row NDJSON imports. The
-- exporter and manifest own sequence integrity; this table is immutable after
-- its extraction transaction, so retain the query index without a PK index.

DROP MACRO analysis.entity_property_history;
DROP MACRO analysis.entity_state_at_game_time;

CREATE TABLE raw.entity_property_updates_v2 (
...
INSERT INTO raw.entity_property_updates_v2
SELECT * FROM raw.entity_property_updates;

DROP TABLE raw.entity_property_updates;
ALTER TABLE raw.entity_property_updates_v2 RENAME TO entity_property_updates;
...
base_properties AS (
    SELECT p.property_path, p.value_type, p.value, p.sequence,
        NULL::BIGINT AS demo_tick, NULL::BIGINT AS net_tick,
        p.checkpoint_game_time AS game_time, 'checkpoint'::VARCHAR AS source
    FROM analysis.checkpoint_properties AS p
    JOIN eligible_checkpoint AS c ON p.sequence = c.sequence
    WHERE p.extraction_id = requested_extraction_id
      AND p.entity_instance_id = requested_instance_id
),
later_updates AS (
    SELECT u.property_path, u.value_type, u.value, u.sequence,
        u.demo_tick, u.net_tick, u.game_time, 'update'::VARCHAR AS source
    FROM raw.entity_property_updates AS u
    WHERE u.extraction_id = requested_extraction_id
      AND u.entity_instance_id = requested_instance_id
...
SELECT
    ...
    property_path, value_type, value, sequence AS source_sequence,
    demo_tick AS source_demo_tick, net_tick AS source_net_tick,
    game_time AS source_game_time, source
