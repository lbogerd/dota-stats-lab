...
export interface StackedDamageChartGroup {
  id: string;
  label: string;
  damage: number;
}

export interface StackedDamageChartInterval {
  startSeconds: number;
  endSeconds: number;
  totalDamage: number;
  groups: readonly StackedDamageChartGroup[];
}
...
export function StackedDamageIntervalChart({
  intervals,
  ...
  intervals: readonly StackedDamageChartInterval[];
  ...
  }), [intervalThickness, markId, maximumDamage, maximumTime, minimumTime, rows, series]);
  ...
  const description = descriptionText(orderedIntervals, descriptionLead);
  ...
  return <figure className="min-w-0 rounded-xl border border-[#e0e3da] bg-white p-3 sm:p-4">
    ...
    <Chart
      ...
      className={`${className} mt-3 rounded-lg outline-none focus-within:ring-2 focus-within:ring-[#4f765f] focus-within:ring-offset-2`}
      ariaLabel={ariaLabel}
      ariaDescription={description}
      onFocusGroupChange={updateSelectedInterval}
    />
    <ul className="mt-2 flex min-w-0 flex-wrap gap-x-4 gap-y-1 text-xs text-[#526158]" aria-label={legendAriaLabel}>
...
function buildSeries(intervals: readonly StackedDamageChartInterval[]): DamageSeries[] {
  ...
  for (const interval of intervals) {
    for (const group of interval.groups) {
      const current = totals.get(group.id);
      totals.set(group.id, {
        label: current?.label ?? group.label,
        damage: (current?.damage ?? 0) + group.damage,
      });
    }
  }
  const orderedGroups = [...totals]
    .sort(([leftName, left], [rightName, right]) => right.damage - left.damage
      || left.label.localeCompare(right.label)
      || leftName.localeCompare(rightName));
  const named = orderedGroups.slice(0, MAX_NAMED_SERIES).map(([id, group], index) => ({
    id,
    label: group.label,
    color: COLORS[index]!,
    groupIds: new Set([id]) as ReadonlySet<string>,
  }));
  const remaining = orderedGroups.slice(MAX_NAMED_SERIES);
  if (remaining.length === 0) return named;
  return [...named, {
    id: OTHER_SERIES_ID,
    label: "Other",
    color: COLORS[MAX_NAMED_SERIES]!,
    groupIds: new Set(remaining.map(([id]) => id)),
  }];
}

function buildRows(intervals: readonly StackedDamageChartInterval[], series: readonly DamageSeries[]): DamageChartRow[] {
...
function damageForSeries(interval: StackedDamageChartInterval, series: DamageSeries): number {
  return interval.groups.reduce((total, group) => series.groupIds.has(group.id) ? total + group.damage : total, 0);
}

function formatIntervalRange(interval: StackedDamageChartInterval): string {
...
function descriptionText(intervals: readonly StackedDamageChartInterval[], lead: string): string {
  if (intervals.length === 0) {
    return `${lead} in 30-second intervals. No intervals are available. Use the left and right arrow keys after focusing the chart to inspect intervals.`;
  }
  return `${lead} in 30-second intervals from ${formatDamageTime(intervals[0]!.startSeconds)} to ${formatDamageTime(intervals.at(-1)!.endSeconds)}. Quiet game-time gaps remain visible. Use the left and right arrow keys after focusing the chart to inspect intervals.`;
}
